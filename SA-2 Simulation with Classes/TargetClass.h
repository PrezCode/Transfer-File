#include <iostream>
#include <string>
#include <cmath>
#include <math.h>
using namespace std;

class Target{
    public:
    Target(double bearing, double range, double heading, double speed, double altitude){   //Generate initial target information
        beta = (450 - bearing)*(M_PI/180);
        alpha = (450 - heading)*(M_PI/180);
        epsilon = atan2(altitude, range); 
        range_ground = range*1000;
        height = altitude*1000;               
        range_slant = sqrt(pow(range, 2) + pow(altitude, 2))*1000;
        X = range_ground*cos(beta);
        Y = range_ground*sin(beta);
        Vx = speed*cos(alpha);
        Vy = speed*sin(alpha);         
    }
    void moveX(double t){X = X + Vx*t;}
    void moveY(double t){Y = Y + Vy*t;}
    void moveRangeGround(){range_ground = sqrt(pow(X, 2) + pow(Y, 2));}
    void moveRangeSlant(){range_slant = sqrt(pow(range_ground, 2) + pow(height, 2));}
    void moveBeta(){beta = -atan2(X, Y);}
    void moveEpsilon(){epsilon = atan2(height, range_ground);}
    double getBeta(){return beta;}
    double getEpsilon(){return epsilon;}
    double getRangeCross(){
        if(beta <= M_PI){
            betaReciprocal = beta + M_PI;
        }else{
            betaReciprocal = beta - M_PI;
        }
        if(betaReciprocal < alpha){
            theta = M_PI - (M_PI/2 + alpha + (2*M_PI - betaReciprocal));
        }else if(betaReciprocal > alpha){
            theta = M_PI - (M_PI/2 + betaReciprocal + (2*M_PI - alpha));
        }else{
            theta = 0;
        }
        if(theta == 0){
            return 0;
        }else if(M_PI/2 + theta < M_PI){
            return range_cross = range_ground*cos(theta);
        }
    }
    double getHeight(){return height;}
    double getRange(){return range_ground;}
    double getSlant(){return range_slant;}
    double getTargetX(){return X;}
    double getTargetY(){return Y;}
    double getVx(){return Vx;}
    double getVy(){return Vy;}
    private:
    double range_ground, range_slant, range_cross, height, X, Y, Vx, Vy, Vz;
    double beta, epsilon, betaReciprocal, alpha, theta;
};