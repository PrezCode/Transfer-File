...
int main(){
    ...
    Missile Launch1(distr(gen)), Launch2(distr(gen)), Launch3(distr(gen));
    ...
    if(azimuth == -1){
        ...
        if(missileLaunch == true){
            ...
            if(Launch1.getTOF() > 3 && Launch1.getTOF() <= 42){Launch1.sustainer(timeStep);}
            if(Launch1.getTOF() > 42){Launch1.glide(timeStep);}
            time += timeStep;
            }
